...
const WhatsAppIcon = () => (
  <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 mr-1.5" fill="currentColor" viewBox="0 0 16 16"> {/* Ukuran ikon h-4 w-4 */}
...
const OrderPopup = ({ shop }) => {
  ...
  const [customerPhoneNumber, setCustomerPhoneNumber] = useState(''); // State untuk nomor HP pelanggan
  ...
  const handleQuantityChange = (itemId, newQuantity) => {
    ...
    setOrderMessage('');
    setOrderSubmitted(false); // Reset status submit jika kuantitas diubah
  };
  ...
  const handleSubmitOrder = async () => {
    ...
    // Opsional: Validasi nomor HP pelanggan di sini jika ingin wajib diisi
    // if (!customerPhoneNumber.trim()) {
    //   setOrderMessage('Nomor HP pelanggan wajib diisi.');
    //   return;
    // }
    ...
    const orderPayload = {
      ...
      totalAmount,
      customerPhoneNumber: customerPhoneNumber.trim() || null, // Kirim nomor HP (atau null jika kosong)
      status: "tertunda",
      timestamp: new Date().toISOString(),
    };
    console.log('OrderPopup.jsx: orderPayload:', orderPayload);
    ...
    try {
      ...
      setOrderItems({});
      setCustomerPhoneNumber(''); // Kosongkan input nomor HP setelah berhasil
            ...
            <div key={item.id || `menu-item-${index}`} className="flex flex-col items-center p-3 bg-slate-50 rounded-lg shadow border border-slate-200 space-y-1">
              ...
              <div className="flex items-center justify-center space-x-1.5 pt-1">
                <button 
                  ...
                  disabled={(orderItems[item.id] || 0) === 0 || isSubmitting || orderSubmitted} 
                ...
                <input 
                  ...
                  className="w-10 sm:w-12 text-center font-semibold border-gray-300 rounded-md focus:ring-purple-500 focus:border-purple-500 p-1 sm:p-1.5 text-sm sm:text-base mx-0.5 sm:mx-1 disabled:bg-gray-100"
                  aria-label={`Jumlah saat ini ${item.name}`}
                />
                <button 
                  onClick={() => incrementQuantity(item.id)} 
                  disabled={isSubmitting || orderSubmitted} 
                  className="p-1.5 sm:p-2 rounded-md text-white bg-purple-500 hover:bg-purple-600 focus:outline-none focus:ring-2 focus:ring-purple-400 focus:ring-opacity-70 transition-colors disabled:bg-gray-300 disabled:cursor-not-allowed"
      ...
      {!orderSubmitted && (
        <div className="mt-4 mb-4">
          ...
          <input
            ...
            id="customerPhone"
            name="customerPhone"
            className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-purple-500 focus:border-purple-500 sm:text-sm disabled:bg-gray-100"
      ...
      {!orderSubmitted ? (
        ...
        >
         ...
         <div className="text-center p-3 bg-blue-100 text-blue-700 border border-blue-300 rounded-md mt-4">
          ...
          {/* Anda bisa tambahkan tombol untuk memesan lagi atau kembali ke peta */}
        </div>
      )}
